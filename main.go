...
var secretsLists = []string{
	...
	"EAACEdEose0cBA[0-9A-Za-z]+", "key-[0-9a-zA-Z]{32}",
	"SK[0-9a-fA-F]{32}",
	"AC[a-zA-Z0-9_\\-]{32}",
	"AP[a-zA-Z0-9_\\-]{32}",
	...
	"(?i)[\"']?linkedin[_-]?client[_-]?secretorlottie[_-]?s3[_-]?api[_-]?key[\"']?[^\\S\r\n]*[=:][^\\S\r\n]*[\"']?[\\w-]+[\"']?",
...
func req(url string) {
	...
	for _, secret := range secretsLists {
...
func banner() {
	...
	██║ ╚═╝ ██║██║  ██║██║ ╚████║   ██║   ██║  ██║██║  ██║
	╚═╝     ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝   ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═╝
			   ` + "\033[31m[\033[37mCoded by MrEmpy\033[31m]\n" +
